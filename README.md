...
> 以下截图为本地 Docker 环境实机运行效果（演示账号数据）。

| 对话与上下文注入 | 上下文预算面板 |
|---|---|
| ![对话](docs/images/01-chat.png) | ![上下文预算面板](docs/images/02-context-panel.png) |
| 流式对话，可附文件/表格（pdf/docx/md/txt/csv/xlsx）随问强制注入，长期记忆与知识库自动装配上下文 | 七层上下文区块的 token 预算与纳入条数逐块可见 |

| 记忆工作台 | 知识库与 RAG 检索 |
|---|---|
| ![记忆工作台](docs/images/03-memories.png) | ![知识库](docs/images/04-knowledge.png) |
| 对话自动抽取的记忆：可筛选、编辑、检索测试 | 文档切片向量化，检索调试与对话 RAG 同口径 |

| 工具风险分级 | token 用量统计 |
|---|---|
| ![工具](docs/images/05-tools.png) | ![用量](docs/images/06-usage.png) |
| 内置工具 + MCP 接入，出网动作需用户确认 | token 成本、上下文区块占比与按日趋势 |
