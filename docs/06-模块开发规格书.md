...
- **fix_v13 检索分区保底 + 判定口径双桶化（65 FAIL 分桶取证闭环）**：
  ...
  保底 ×1、双桶口径 ×3）。**本轮验证口径说明**：0.59 系容器内生产检索
  链路（retrieve 真实调用 + _hit_in_top 判定）对 fix_v12_mh 评测账号
  库全量 100 行重放实测，未走完整评测管线（省一次全量 LLM 消耗）；
  fix_v13_mh 官方报告留待下次改动（B1 线抽取保真）合并重跑。
  遗留（后续专项）：库内 conflicted 66/194
  （34%，抽取/仲裁污染治理）、C1 改写粒度 14 行（expected 字面 vs
  抽取改写）、C3 完全缺失 9 行（抽取漏抽）、lt 生成层复述问题。
- **B1 线抽取保真 + 溯源后验过滤（fix_v13 遗留的抽取层治理）**：
  ①`EXTRACT_SYSTEM` 保真硬规则——数字/金额/时长/日期/时段**原样保留
  用户表述**（"21k"不写"21000"、"两万五"不写"25000"、"两小时"不写
  "2小时"，不做换算/四舍五入/单位转换/省略），专名保留原文不意译；
  治 fix_v13_lt 实锤的薪资数字/时间改写必丢（4 组 NOT_IN_LIB 全是
  薪资数字）与 C1 改写粒度 14 行。静态断言锁定防 prompt 回退。
  ②`pipeline._filter_untraceable` 溯源后验过滤——代码级防脑补：候选
  必须携带有效 source_message_ids（非空且指向本轮真实消息），空引用
  丢弃、编造 ID 剔除（剔除后全空则丢弃、有合法部分则保留合法部分）；
  fix_v11 问句约束是 prompt 层防线（对 qwen-plus 非全量遵从），本防线
  在代码层强制"记忆必须可溯源"，同时保住 memories.source_message_ids
  的可追溯产品承诺。结构性校验零语义误杀（真实事实的源消息必然在本轮
  消息中）。回归用例 2（空引用+编造引用丢弃 / 混合引用剔除保留）。
  过滤位置在 confidence 过滤之后、向量化之前（省无效 embedding 调用）。
...
```
app/rag/
├── parser.py        # pypdf/python-docx/openpyxl/csv/md/txt → 纯文本（白名单外 422）
│                    #   表格类（csv/xlsx）渲染为「表头 + 逐行 列名: 值」，多编码兜底
...
│                    #   fetch_by_file_ids：聊天附件强制取全量切片（高分置顶，绕过相关性检索）
...
app/context/builder.py             # RAG 区块注入 + citations 与预算对齐 + 聊天附件强制注入
evals/rag_recall.py + data/        # Recall@K 冒烟评测（10 条查询）
alembic 5de8d97c8e0d               # embedding/tokens/workspace_id + HNSW 索引
tests/test_rag_pipeline.py         # 24 例（含 csv/xlsx 解析、附件强制注入）
```
...
app/api/routes/chat.py           # POST /v1/chat/completions（enable_tools 开关 + attachment_ids 附件直注）+ POST /v1/chat/resume
...
- **聊天附件（`metadata.attachment_ids`）**：前端先经 `/api/knowledge/files` 上传（复用 M-07 ingest 管线，同步解析切片），再把返回的文件 ID 随对话请求带上；`attachment_ids` 经路由 → GraphContext → `load_context` → `ContextBuilder.build`，由 `KnowledgeRetriever.fetch_by_file_ids` 按 workspace 隔离直取该文件**全部切片**并赋 `FORCED_ATTACHMENT_SCORE=2.0`（高于任何检索命中），保证预算内优先装入 RAG 区块并进入 `metadata.citations`。设计取舍：附件即知识文件（持久化、可复用、可溯源），非临时态；跨 workspace 的文件 ID 静默跳过（防越权枚举）；取附件失败降级为空，不阻断对话
...
app/tools/mcp_client.py         # MCP 接入（M-08 M3 落地：连接池/动态注册/降级，详见 §10；热插拔见下）
app/models/mcp_server.py        # McpServer ORM（mcp_servers 表：name 唯一 + source=env/user）
app/repositories/mcp_server_repo.py  # list_all/list_enabled/get_by_name/create/delete（env 种子清理）
app/services/mcp_server_service.py   # bootstrap（env 种子同步+全量连接）/add（试连后落库）/remove
app/api/routes/mcp.py           # GET/POST/DELETE /api/mcp/servers（热插拔管理）
...
tests/test_mcp_client.py        # MCP 连接/参数模型/热插拔（unregister_prefix/热移除）
tests/test_mcp_servers_api.py   # /api/mcp/servers 6 例（鉴权/CRUD/422/502/env 保护/bootstrap 种子）
```
...
- **MCP 服务热插拔（运行时管理，进程级全局生效）**：server 配置持久化在 `mcp_servers` 表（迁移 c7335478c008 + 4327e83bfc63），`source=env` 行由启动期 `bootstrap` 从 `MEMORY_SERVICE_MCP_SERVERS_JSON` 同步（增改删对齐，API 不可删——400 `mcp_env_managed` 指引改环境变量后重启），`source=user` 行经 `POST /api/mcp/servers` 运行时添加；添加走**先试连后落库**（连接失败 502 `mcp_connect_failed`，不留脏行；重名 409；配置非法 422 `mcp_config_invalid`），成功即 `list_tools` 并按 `mcp.<server>.<tool>` 前缀热注册；`DELETE /api/mcp/servers/{name}` 断连 + `registry.unregister_prefix` 热卸载 + 删行（审计日志保留）；`GET /api/mcp/servers` 回带 `connected/tools` 实时态。前端 Tools 页「MCP 服务管理」区块闭环（2026-09-23 实机：stdio demo server 添加→`mcp.demo-mcp.echo` 注册→external 确认流执行 success→移除后注册表即时消失）
- **托管 MCP 鉴权接入（headers + sse transport）**：transport 三选一 `http|sse|stdio`（sse 为百炼/高德等托管端点常用形态）；`headers` 字段（JSONB，迁移 4327e83bfc63）承载 API key 等自定义请求头——http 经 `create_mcp_http_client(headers=...)` 注入、sse 直传 `sse_client(headers=...)`；**密钥不回显**：`GET /api/mcp/servers` 的 headers 值统一打码 `***`（DB 存原值），前端展示「鉴权头」徽标；单测以 monkeypatch 探针断言 headers 确实到达 transport 工厂（`test_run_injects_headers_into_transport`）
