...
>
> 一句话定位：自研前端工作台（React + TypeScript）+ 自研长期记忆与上下文组装服务（memory-service），支持跨会话记忆、知识库问答、MCP 工具调用、token 成本控制和 Langfuse 观测。
>
> 上游来源声明：**全部代码自研**（前端 `frontend/`、服务 `services/memory-service/`）。历史沿革：阶段1 曾采用 [Open WebUI](https://github.com/open-webui/open-webui) 官方镜像（未改源码）+ 自研 Pipe 桥接函数做产品壳验证，阶段2（M-F6）起替换为自研前端，Open WebUI 容器与 Pipe 已退役；仓库不含 Open WebUI 源码，无二开代码。
>
> 文档版本：v1.1（2026-09-20，M-F6 起前端自研、Open WebUI 退役，现状表述全文同步；v1.0 2026-09-18 初版）
...
**自研前端（React SPA，Nginx 托管）与自研 memory-service（Python / FastAPI）经同源反代直连**：浏览器请求 `/api`（REST）与 `/v1`（OpenAI-compatible SSE）由 Nginx 反代到 memory-service，完成记忆检索、上下文组装、RAG、工具调用与生成。

> 历史沿革（阶段1）：早期用 Open WebUI 官方镜像做产品壳，经其 Pipe/Function 机制转发到 memory-service；该方案的 chat→session 映射存进程内存、多用户共用服务账号、确认流靠文本协议三类限制随前端自研（阶段2，见 docs/07）全部消除，Open WebUI 已退役（Pipe 脚本归档于 `deploy/pipes/`）。

好处：

1. 前后端均为自研，无上游升级冲突与 license 义务；
2. 简历可如实表述"自研前端工作台与记忆服务、上下文组装全栈"；
...
```mermaid
flowchart TD
    U[用户浏览器] --> UI[EchoDesk 前端<br/>React SPA · 会话/知识库/记忆/任务/用量/设置]

    UI -->|Nginx 同源反代 /api /v1| GW[memory-service · FastAPI Gateway]
...
| 层 | 选型 | 用途说明 |
|---|---|---|
| 前端工作台 | React 18 + TypeScript + Vite + Tailwind（自研） | 聊天 UI、会话列表、文件上传、用户体系；Nginx 托管并同源反代自研服务（见 docs/07） |
...
| Embedding | BGE-M3（本地，infinity 服务承载） | 记忆向量 + 文档向量统一；OpenAI 兼容 /embeddings，通义 API 为备选 |
| 向量库 | pgvector（Milvus 预留） | 记忆检索；知识库检索（HNSW 余弦索引） |
...
| 异步任务 | 进程内 asyncio.create_task | 记忆抽取等后台任务（M-04 起不引入 Celery/ARQ，量大再演进） |
| 部署 | Docker Compose | 一键起 web（前端）+ memory-service + PG + Redis + Milvus + Langfuse |
| 前端测试 | vitest + testing-library | SSE 解析、JWT 拦截器、确认流状态机、页面冒烟 |
...
| `sessions` | 会话 | id, workspace_id, user_id, title, status(active/archived), rolling_summary, token_total, last_message_at, version(乐观锁), created_at |
...
对话轮次（user + assistant 消息）
  → ① LLM 结构化抽取（JSON schema：fact / preference / task_state / event，含 key、confidence；
       注入 ws 既有 active key 列表引导复用命名——同主题必须复用已有 key，仅新主题新建）
  → ② 打分（confidence × importance × recency 衰减）
  → ③ 去重（三层：key 精确 → 向量相似度 ≥ 0.92 判重 → 0.80~0.92 疑似冲突层仅送仲裁禁合并）
       ├─ 语义一致 → 合并并提升 confidence（仅判重层允许）
       └─ 语义不一致 → 冲突处理
  → ④ 冲突处理（LLM 仲裁：新事实覆盖旧事实 → 旧记忆 status=superseded 并挂版本链；
       无法判定 → 双条标 conflicted，等待用户在记忆面板裁决）
  → ⑤ 写入（UPSERT + memory_events 流水；候选逐条独立提交，死锁回滚重试一次）
```
...
1. 认证：JWT（access + refresh），前端直连持有自有账号体系（阶段1 Pipe 服务账号映射方案随 Open WebUI 退役）；
...
2. 前端：自研 React 记忆工作台页（列表 + 过滤 + 冲突置顶 + 详情抽屉编辑/软删/裁决 + 版本链与事件流水，M-F4 已实现）；
...
### 6.1 Chat（OpenAI-compatible，供自研前端调用，M-08 已实现）

| Method | Path | 说明 |
|---|---|---|
| POST | `/v1/chat/completions` | SSE 流式；兼容 OpenAI 格式，请求 `metadata` 携带 workspace_id/session_id/enable_tools；session_id 缺省自动建会话；**响应（流式首片/finish 片与非流式体）回传 `metadata.session_id`**，客户端凭它延续多轮会话；用户/回答消息自动落库并写入 Working Memory 窗口。enable_tools=True（默认）走 LangGraph 工具循环：write/read_only 工具直执行并回灌，external 工具先落 PENDING 审计 + SSE `metadata.tool_call` 事件（含 call_id/tool/args/risk）后 interrupt 挂起，等 `/v1/chat/resume` 裁决 |
| POST | `/v1/chat/resume` | external 工具确认流恢复（M-08 M3）：请求体 `{workspace_id, session_id, call_id, approve}`；服务端从 Postgres checkpoint（thread = `workspace_id:session_id`，落 `agent_ckpt` schema）恢复挂起的图——approve 执行工具并续传剩余回答 / deny 回灌拒绝说明由 LLM 收敛；SSE 事件与 completions 同构（可连续携带新的 tool_call 确认请求）。服务重启后仍可恢复（checkpoint 持久化）；重复裁决已终态的 call_id 返回 409 |
| POST | `/v1/tasks/completions` | 无状态任务补全（评测 LLM-as-judge 调用等）：纯 LLM 透传，不建会话、不落库，避免任务 prompt 污染会话历史；返回标准 chat.completion JSON（含真实 usage）。历史：曾供 Open WebUI 标题/跟进生成（现已随其退役），端点保留 |

> M-08 M3 收官：上下文组装（记忆/RAG）与工具循环均在 LangGraph 图内执行，迭代 ≤8 轮强制收敛；工具分级权限与审计见 §5.6，checkpoint 降级策略（PG 不可达回落内存、重启丢恢复能力但对话不受影响）见 docs/06 §10。
> LLM 经 OpenAI 兼容协议调用（`deploy/.env` 配置 `LLM_BASE_URL/API_KEY/MODEL`）；未配置返回 503，上游失败返回 502（流式以 error 事件收尾）。

### 6.2 认证与 workspace（M-02 已实现）

| Method | Path | 说明 |
|---|---|---|
| POST | `/api/auth/register` | 注册（201）；单事务创建用户 + 个人 workspace + owner 成员；同名 409 |
| POST | `/api/auth/login` | 登录换取 access(30min)/refresh(7d) 双令牌；失败统一 401 防枚举 |
| POST | `/api/auth/refresh` | refresh 令牌换发新令牌对；类型校验（access 冒充 refresh → 401） |
| GET | `/api/auth/me` | 当前用户信息（access 令牌自省） |
| GET | `/api/workspaces` | 当前用户所属 workspace 列表 |
| GET | `/api/workspaces/{ws_id}` | workspace 详情（成员可见；非成员/不存在统一 403 防存在性泄露） |
| GET | `/api/workspaces/{ws_id}/members` | 成员列表（ADMIN+） |
| POST | `/api/workspaces/{ws_id}/members` | 添加/更新成员（ADMIN+；禁止指定 OWNER） |
...
| PATCH | `/api/sessions/{id}` | 重命名 / 归档 / 恢复（删除走 DELETE） |
| DELETE | `/api/sessions/{id}` | 删除（可选级联清理记忆） |
| GET | `/api/sessions/{id}/messages` | 消息历史（含引用与 trace 链接） |
| POST | `/api/sessions/{id}/messages` | 追加消息（Redis 会话锁 + 乐观锁，冲突 409） |
...
| Fact Conflict Rate | 冲突场景中新旧事实以两条独立条目并存于上下文的比例（supersede 后新条目以背景口吻提及旧值属良性改写，不计入） | ≤ 10% |
...
```
ai-long-workbench/
├── frontend/                      # ★ 自研前端（React 18 + TS + Vite + Tailwind）
│   ├── src/api/  src/stores/  src/pages/  src/lib/   # HTTP 层 / Zustand / 页面 / SSE 解析
│   ├── Dockerfile  nginx.conf     # 多阶段构建 + Nginx 托管与同源反代（M-F6）
│   └── scripts/                   # 实机验证探针（Python，经 dev proxy 或 3000）
├── services/
...
│       ├── evals/                 # ★ 评测集 + 评测脚本 + 报告
│       ├── tests/                 # 单元 / 集成 / 隔离测试
│       └── config/                # 预算策略 YAML（支持 A/B 多套）
├── deploy/
│   ├── docker-compose.yml         # 全栈编排（web / memory-service / PG / Redis / embedding / Langfuse）
│   ├── pipes/                     # 阶段1 Open WebUI Pipe 桥接脚本（已退役，归档追溯）
│   └── .env.example
...
1. **来源诚实**：全部代码自研（README 模块来源表如实声明）；阶段1 曾用 Open WebUI 官方镜像做壳（未改其源码、无二开代码），阶段2 起已退役并在 README/docs 保留沿革说明，不涉及其 license 义务；
2. **自研边界诚实**：简历与面试中如实描述架构演进（阶段1 Open WebUI 壳验证 → 阶段2 前端自研直连），绝不夸大；
3. **数据合规**：求职数据仅本人使用；Demo 录屏脱敏。

### 模块来源表（README 必含）

| 模块 | 来源 |
|---|---|
| 前端工作台（聊天 UI / 记忆工作台 / 知识库 / 任务 / 用量 / 设置） | 自研（React 18 + TS + Vite + Tailwind） |
| 会话与消息存储 | 自研 |
...
**AI 长会话知识工作台 EchoDesk（个人项目，全栈自研）**

- 项目描述：面向求职复习与技术学习场景，构建支持多会话、长期记忆、知识库问答和 MCP 工具调用的 AI 工作台；自研 React 前端工作台与 FastAPI 记忆服务，解决长对话上下文膨胀、跨会话信息丢失、无关记忆污染回答等问题。
- 技术栈：React 18、TypeScript、Vite、Tailwind、Python、FastAPI、LangGraph、PostgreSQL（pgvector）、Redis、MCP、Langfuse、Docker Compose、Nginx
- 核心工作：自研前端（SSE 流式聊天 / 记忆工作台 / RAG 调试）/ 四层记忆模型 / 长对话上下文管线 / 记忆抽取与冲突处理 / workspace 隔离 / MCP + Langfuse / 50 轮长对话评测
- 项目成果：Memory Recall@5 XX% / token 成本下降 XX% / Task Continuity XX% / P95 XX s

### 12.2 面试必答题准备

1. **"是不是开源包装？"** → 标准话术：全部代码自研（前端 React/TS 工作台 + memory-service 四层记忆、抽取冲突管线、token 预算组装、隔离与观测）；早期曾用 Open WebUI 官方镜像做壳验证链路，后因多租户/确认流限制替换为自研前端——这也正是架构演进的判断力体现。
