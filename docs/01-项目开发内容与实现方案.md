...
### 6.1 Chat（OpenAI-compatible，供 Open WebUI Pipe 调用，M-08 已实现）
...
| POST | `/v1/tasks/completions` | 无状态任务补全（Open WebUI 标题/跟进生成等）：纯 LLM 透传，不建会话、不落库，避免任务 prompt 污染会话历史；返回标准 chat.completion JSON（含真实 usage） |
