...
### 6.1 Chat（OpenAI-compatible，供 Open WebUI Pipe 调用，M-08 M1 已实现）

| Method | Path | 说明 |
|---|---|---|
| POST | `/v1/chat/completions` | SSE 流式；兼容 OpenAI 格式，请求 `metadata` 携带 workspace_id/session_id/用户标识；session_id 缺省自动建会话；**响应（流式首片/finish 片与非流式体）回传 `metadata.session_id`**，客户端凭它延续多轮会话；用户/回答消息自动落库并写入 Working Memory 窗口 |
...
> M1 直通阶段：上下文 = Working Memory 窗口；记忆检索/RAG/工具循环在 M3 阶段接入 LangGraph 完整图。
